...
export function SidebarLayout({ children, user, household, memberCount, activeTab, setActiveTab }: {
    ...
    // Use Household Image OR App Logo as fallback
        ...
        <div className="flex min-h-screen bg-slate-50 relative">
            ...
            <aside className="hidden md:flex w-72 flex-col border-r border-white/40 bg-white/60 backdrop-blur-xl h-screen sticky top-0 z-30 shadow-sm">
                <div className="p-6 pb-2">
                    {/* APP LOGO -> SETTINGS */}
                    <div className="mb-8 pl-1 flex items-center gap-2 cursor-pointer hover:opacity-70 transition-opacity" onClick={() => setActiveTab('settings')}>
                    ...
                    {/* HOUSEHOLD HEADER -> SETTINGS */}
                    <div className="flex items-center gap-3 p-3 bg-white/50 rounded-xl border border-white/60 shadow-sm backdrop-blur-md cursor-pointer hover:bg-white/80 transition-colors" onClick={() => setActiveTab('settings')}>
                ...
                <nav className="flex-1 px-4 space-y-1.5">
                    {navItems.map(item => {
                        ...
                        return (
                            <button key={item.value} onClick={() => setActiveTab(item.value)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ease-out group ${isActive ? 'bg-slate-900/90 text-white shadow-lg shadow-slate-900/10 scale-[1.02] backdrop-blur-md' : 'text-slate-600 hover:bg-white/60 hover:text-slate-900 active:scale-95'}`}>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/60 cursor-pointer transition-all active:scale-95 duration-200">
                                ...
                                <div className="flex-1 overflow-hidden text-left"><p className="text-sm font-semibold text-slate-700 truncate">{displayName}</p><p className="text-[10px] text-slate-400 truncate">My Account</p></div>
            ...
            <main className="flex-1 w-full min-w-0 pb-24 md:pb-8 relative z-10">
                <header className="md:hidden sticky top-0 z-50 bg-white/70 backdrop-blur-xl border-b border-white/40 px-4 py-3 flex justify-between items-center shadow-sm">
                    <div className="flex items-center gap-2.5 cursor-pointer active:opacity-70 transition-opacity" onClick={() => setActiveTab('settings')}>
                ...
                <div className="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto animate-in fade-in duration-700">{children}</div>
            </main>
                    ...
                    {navItems.map(item => {
                        ...
                        return (
                            <button key={item.value} onClick={() => setActiveTab(item.value)} className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-all duration-200 active:scale-90 feedback-button`}>
                                <div className={`p-1.5 rounded-full transition-colors duration-300 ${isActive ? 'bg-slate-900 text-lime-400 shadow-lg shadow-slate-900/20' : 'text-slate-400'}`}><item.icon className="w-5 h-5" /></div>
